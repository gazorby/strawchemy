...
class AliasContext(Generic[DeclarativeT]):
    ...
    def detached(self, alias: AliasedClass[Any]) -> AliasContext[Any]:
        """Creates a root context on ``alias``, sharing no alias with this one."""
        return AliasContext(inspect(alias).mapper.class_, self.dialect, root_alias=alias, inspector=self._inspector)
