...
class StrawchemyUpdateMutationField(_StrawchemyInputMutationField, _StrawchemyMutationField):
    ...
    def auto_arguments(self) -> list[StrawberryArgument]:
        ...
        data_annotations = (
            StrawberryAnnotation(list[self._input_type]) if self.is_list else StrawberryAnnotation(self._input_type)
        )
        return [StrawberryArgument(DATA_KEY, None, type_annotation=data_annotations)]
