"""Compile-only, DB-free tests for the QueryPlan IR emitter (emit_plan)."""
...
def test_emit_plan_assembles_select_where_order() -> None:
    """emit_plan builds SELECT/WHERE/ORDER BY from plan leaf fragments."""
    fruit = aliased(Fruit)
    plan = QueryPlan(
    ...
    compiled = str(emit_plan(plan).compile(dialect=sqlite.dialect()))
...
def test_emit_plan_without_where_or_order() -> None:
...
def test_emit_plan_applies_filter_semijoin() -> None:
...
def test_emit_plan_appends_root_aggregation_columns() -> None:
...
def test_emit_plan_applies_limit_and_offset() -> None:
...
def test_emit_plan_applies_native_distinct_on() -> None:
    ...
    compiled = str(emit_plan(plan).compile(dialect=postgresql.dialect()))
    assert "DISTINCT ON" in compiled
    assert "color_id" in compiled


def test_emit_plan_replays_hook_specs() -> None:
    """emit_plan replays hook specs via HookApplier, applying apply_hook to the statement.
